...
class MNISTModel(LightningModule):
    ...
    def on_test_epoch_end(self):
        predictions = torch.cat(self.test_step_pred)
        y = torch.cat(self.test_step_y)
        good_predictions = torch.sum(y == predictions).item()
        self.log("Test accuracy", 100*good_predictions/y.shape[0])
        self._compute_and_display_confusion_matrix(predictions, y, NUM_CLASSES)

    def _compute_and_display_confusion_matrix(self, predictions, y, nb_classes): 
        # Compute confusion matrix and display it in Tensorboard
        device = "cuda" if torch.cuda.is_available() else "cpu"
        confusion = MulticlassConfusionMatrix(num_classes=nb_classes).to(device)  
        confusion.update(predictions, y)
        fig, ax = confusion.plot()
        # save the plot in a tensor
        buf = io.BytesIO()
